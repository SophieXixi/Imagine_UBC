export class Section {
	...
	constructor(id: string, Course: string, Title: string, Professor: string, Subject: string,
		Year: number, Avg: number, Audit: number, Pass: number, Fail: number ) {
		this.uuid = id;
		this.id = Course;
		this.title = Title;
		...
		return new Promise((resolve, reject) => {
			try {
				...
				for (const result in raredata.result){
					try{
						if ((raredata.id || raredata.Course || raredata.Title || raredata.Avg ||
							raredata.Professor || raredata.Subject || raredata.Year ||
							raredata.Audit || raredata.Pass || raredata.Fail) === undefined){
